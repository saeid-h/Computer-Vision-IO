...
def read_flow_png(flow_file):
    ...
    flow_object = read_image(flow_file)
...
def flow_error_map (tu, tv, u, v, treshhold = 1.0):
    """
    Calculate average end point error
    :param tu: ground-truth horizontal flow map
    :param tv: ground-truth vertical flow map
    :param u:  estimated horizontal flow map
    :param v:  estimated vertical flow map
    :return: Error map of the estimated flow
    """
    smallflow = SMALLFLOW
    '''
    stu = tu[bord+1:end-bord,bord+1:end-bord]
    ...
    epe = np.sqrt((stu - su) ** 2 + (stv - sv) ** 2)
    epe = epe[ind2]
    epe = np.reshape(epe, u.shape)
    treshhold = np.mean(epe)
    epe[epe<treshhold] = 0.0
    epe[epe>=treshhold] = 1.0
    epe = epe * 255
    return epe.astype('uint8')
...
def read_disp_png(file_name):
    ...
    image_object = read_image(file_name)
